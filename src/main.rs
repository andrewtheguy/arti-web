//! POC: a Tor onion (hidden) service, built on Arti, that gates "restricted
//! content" behind a regular web login (form POST + server-side session cookie).
//!
...
//! under the data dir (default `./arti-web-data`, override `ARTI_WEB_DATA_DIR`),
//! so the .onion address stays the same across restarts. Guard that directory —
//! its key *is* the service identity.
//!
//! Run with `RUST_LOG=info cargo run`, then open the printed http://<...>.onion
//! URL in Tor Browser. Demo credentials are printed to the console at startup.
...
/// Cookie attributes. `Secure` is honored because Tor Browser treats `.onion`
/// as a secure context. `SameSite=Strict` blocks cross-site requests (CSRF).
/// If sessions ever fail to stick on a non-Tor-Browser client, drop `Secure`.
...
async fn main() -> Result<()> {
    ...
    // identity), so the directory is created 0700 and fs-mistrust checks stay on.
    // Override the location with ARTI_WEB_DATA_DIR. ---
    let data_dir = std::env::var_os("ARTI_WEB_DATA_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from("arti-web-data"));
